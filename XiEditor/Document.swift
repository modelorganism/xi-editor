...
enum Action {
    ...
    var params: AnyObject {
        ...
        case .Search(let str, let flags):
            return ["text": str, "flags": flags]
        case .UpdateSearch(let str, let flags):
            return ["text": str, "flags": flags]
        case .FindNext:
            return [:]
        case .FindPrev:
...
class Document: NSDocument {
    ...
    */
    ...
    override func makeWindowControllers() {
        ...
        editView = (windowController.contentViewController as? EditViewController)?.editView
        editView?.document = self
    ...
    /// Update the search string or parameters.
    func updateSearch(searchText:String, params: SearchParams ) -> FindStrResponse {
        
        let flags = (params.isRegExp ? 1 : 0) + (params.matchCase ? 2 : 0) + (params.wholeWordOnly ? 4 : 0)
        let res = sendRPCAction(.UpdateSearch(searchText, flags)) as! String
        return res.hasPrefix("ok") ? FindStrResponse.Ok : FindStrResponse.BadGrep
    }
    
    func searchPrev() {
        sendRPCAction(.FindPrev)
    }
    
    func searchNext() {
        sendRPCAction(.FindNext)
    }
}
